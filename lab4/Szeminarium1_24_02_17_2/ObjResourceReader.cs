...
namespace Szeminarium1_24_02_17_2
{
    internal class ObjResourceReader
    {
        public static unsafe GlObject CreateTeapotWithColor(GL Gl, float[] faceColor)
        {
            ...
            ReadObjDataForTeapot(out objVertices, out objFaces, out objNormals);
        ...
        private static unsafe void CreateGlArraysFromObjArrays(float[] faceColor, List<float[]> objVertices, List<int[]> objFaces, List<float[]> objNormals, List<float> glVertices, List<float> glColors, List<uint> glIndices)
        {
            ...
            {
                var faceNormal = objNormals.Count != 0 ? objNormals[faceIndex] : null;
                var aObjVertex = objVertices[objFaces[faceIndex][0] - 1];
                var a = new Vector3D<float>(aObjVertex[0], aObjVertex[1], aObjVertex[2]);
                var bObjVertex = objVertices[objFaces[faceIndex][1] - 1];
                var b = new Vector3D<float>(bObjVertex[0], bObjVertex[1], bObjVertex[2]);
                var cObjVertex = objVertices[objFaces[faceIndex][2] - 1];
                var c = new Vector3D<float>(cObjVertex[0], cObjVertex[1], cObjVertex[2]);

                var normal = Vector3D.Normalize(Vector3D.Cross(b - a, c - a));

                // process 3 vertices
                for (int i = 0; i < objFaces[faceIndex].Length; ++i)
                {
                    var objVertex = objVertices[objFaces[faceIndex][i] - 1];
                    List<float> glVertex = new List<float>();
                    if (faceNormal != null)
                    {
                        glVertex.AddRange(faceNormal);
                        glVertex.Add(faceNormal[0]);
                        glVertex.Add(faceNormal[1]);
                        glVertex.Add(faceNormal[2]);
                    }
        ...
        private static unsafe void ReadObjDataForTeapot(out List<float[]> objVertices, out List<int[]> objFaces, out List<float[]> objNormals)
        {
